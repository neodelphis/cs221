...
grader.addManualPart(
    '1a', 3, description="Written question: value iteration in basic MDP")
grader.addManualPart(
    '1b', 3, description="Written question: optimal policy in basic MDP")
...
print('-' * 80)
grader.addHiddenPart('2a-hidden', test2a, 3,
                     description="Hidden test for CounterexampleMDP. Ensure that V[startState] is greater after noise is added.")

grader.addManualPart(
    '2b', 3, description="Written question: single-pass algorithm for node values in acyclic MDP")
grader.addManualPart(
    '2c', 3, description="Written question: define new MDP solver for discounts < 1")
...
grader.addBasicPart('3a-basic', test3a, 5,
...
grader.addHiddenPart('3a-hidden', test3aHidden, 5,
...
grader.addBasicPart('3b-basic', test3b, 4,
                    description="Test for peekingMDP().  Ensure that in at least 10% of states, the optimal policy is to peek.")
...
grader.addBasicPart('4a-basic', test4a, 5, maxSeconds=10,
                    description="Basic test for incorporateFeedback() using NumberLineMDP.")


def test4aHidden():
    smallMDP = submission.BlackjackMDP(
...
grader.addHiddenPart('4a-hidden', test4aHidden, 3, maxSeconds=5,
...
grader.addManualPart(
    '4b', 4, description="Written question: policy comparison for Q-learning vs. value iteration")
...
def run4dHelper():
    submission.compare_changed_MDP(
