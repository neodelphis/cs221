...
def test3f1():
    trainExamples = readExamples('polarity.train')
    devExamples = readExamples('polarity.dev')
    featureExtractor = submission.extractCharacterFeatures
    weights = submission.learnPredictor(trainExamples, devExamples, featureExtractor(15), numIters=20, eta=0.01)
    #outputWeights(weights, 'weights')
    #outputErrorAnalysis(devExamples, featureExtractor, weights, 'error-analysis')  # Use this to debug
    #trainError = evaluatePredictor(trainExamples, lambda(x) : (1 if dotProduct(featureExtractor(x), weights) >= 0 else -1))
    #devError = evaluatePredictor(devExamples, lambda(x) : (1 if dotProduct(featureExtractor(x), weights) >= 0 else -1))
    #print "Official: train error = %s, dev error = %s" % (trainError, devError)
grader.addBasicPart('3f-1-perso', test3f1, maxSeconds=30, description="test avec des features de type n-gram")
...
grader.addHiddenPart('4b-1-hidden', test4b1, maxPoints=1, description="test stability of cluster assignments")
...
grader.addHiddenPart('4b-2-hidden', test4b2, maxPoints=1,  description="test stability of cluster locations")
