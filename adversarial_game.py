...
class HalvingGame:
	...
	def utility(self, state):
		player, number = state
		assert self.isEnd(state) # utility est seulement valide au dernier état
...
def human_policy(game, state):
	while True:
		action = input('tour {}: entrer l\'action choisie - ou / : '.format(state))
...
def minimax_policy(game, action):
	...
	print('minimax policy: state {} => action {} with utility {}'.format(state, action, utility))
